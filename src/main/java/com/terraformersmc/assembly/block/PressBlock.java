...
public class PressBlock extends HorizontalFacingBlock implements BlockEntityProvider, AttributeProvider {
	...
	public void addAllAttributes(World world, BlockPos pos, BlockState state, AttributeList<?> to) {
		if (state.get(HALF) == DoubleBlockHalf.LOWER) {
			...
			if (blockEntity instanceof PressBlockEntity) {
				PressBlockEntity steamPress = (PressBlockEntity) blockEntity;
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		return Interactions.handleDefaultInteractions(state, world, pos, player, hand, hit, Interaction.HANDLE_FLUIDS, (state1, world1, pos1, player1, hand1, hit1) -> {
			if (world != null) {
				...
				if (blockEntity instanceof PressBlockEntity) {
					PressBlockEntity steamPress = ((PressBlockEntity) blockEntity);
					ItemStack outputStack = steamPress.getStack(PressBlockEntity.OUTPUT_SLOT);
					if (!outputStack.isEmpty()) {
						if (!player.isCreative()) {
							player.inventory.insertStack(outputStack);
						}
						steamPress.removeStack(PressBlockEntity.OUTPUT_SLOT);
						...
						if (steamPress.canInsert(PressBlockEntity.INPUT_SLOT, player.getStackInHand(hand).copy().split(1), hit.getSide())) {
							ItemStack stack;
							if (!player.isCreative()) {
							...
							steamPress.setStack(PressBlockEntity.INPUT_SLOT, stack);
							if (!stack.isEmpty()) {
								if (player instanceof ServerPlayerEntity) {
	...
	public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
		...
		DoubleBlockHalf half = state.get(HALF);
		if (blockEntity instanceof PressBlockEntity) {
			return VoxelShapes.union(half == DoubleBlockHalf.LOWER ? LOWER_SHAPE : UPPER_SHAPE, ((PressBlockEntity) blockEntity).getArmVoxelShape(half));
		}
	...
	@Override
	public Item asItem() {
		return AssemblyItems.PRESS;
	}

	@Nullable
	@Override
	public BlockEntity createBlockEntity(BlockView view) {
		return AssemblyBlockEntities.PRESS.instantiate();
	}
