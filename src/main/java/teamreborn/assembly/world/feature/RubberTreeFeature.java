...
public class RubberTreeFeature extends AbstractTreeFeature<DefaultFeatureConfig> {
	public static final BlockState LOG = AssemblyBlocks.RUBBER_LOG.getDefaultState().with(AssemblyProperties.ALIVE, true);
	public static final BlockState LEAVES = AssemblyBlocks.RUBBER_LEAVES.getDefaultState();

	public RubberTreeFeature(Function<Dynamic<?>, DefaultFeatureConfig> function, boolean worldGen) {
				...
				for (y = 0; y < height; ++y) {
					if (isAirOrLeaves(world, pos.up(y)) || isReplaceablePlant(world, pos.up(y))) {
						if (random.nextInt(5) == 0) {
							this.setBlockState(feature, world, pos.up(y), LOG.with(RubberLogBlock.getRandomLatexProperty(random), true), box);
						} else {
