...
public class WoodenBarrelBlockEntityRenderer extends BlockEntityRenderer<WoodenBarrelBlockEntity> {
	...
	public void render(WoodenBarrelBlockEntity barrel, float delta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		if (barrel.fluidInstance != null && barrel.fluidInstance.getFluid() != Fluids.EMPTY) {
			...
			buffer.vertex(modelMatrix, fracf(3), fracf(height), fracf(3)).color(r, g, b, 1F).texture(sprite.getFrameU(getXFromU(sprite, sprite.getMinU()) + 3), sprite.getFrameV(getYFromV(sprite, sprite.getMinV()) + 3)).light(light).normal(normalMatrix, 0.0F, 1.0F, 0.0F).next();
			buffer.vertex(modelMatrix, fracf(3), fracf(height), fracf(13)).color(r, g, b, 1F).texture(sprite.getFrameU(getXFromU(sprite, sprite.getMaxU()) - 3), sprite.getFrameV(getYFromV(sprite, sprite.getMinV()) + 3)).light(light).normal(normalMatrix, 0.0F, 1.0F, 0.0F).next();
			buffer.vertex(modelMatrix, fracf(13), fracf(height), fracf(13)).color(r, g, b, 1F).texture(sprite.getFrameU(getXFromU(sprite, sprite.getMaxU()) - 3), sprite.getFrameV(getYFromV(sprite, sprite.getMaxV()) - 3)).light(light).normal(normalMatrix, 0.0F, 1.0F, 0.0F).next();
			buffer.vertex(modelMatrix, fracf(13), fracf(height), fracf(3)).color(r, g, b, 1F).texture(sprite.getFrameU(getXFromU(sprite, sprite.getMinU()) + 3), sprite.getFrameV(getYFromV(sprite, sprite.getMaxV()) - 3)).light(light).normal(normalMatrix, 0.0F, 1.0F, 0.0F).next();
		}
	}
}
